...
describe('Listing Users', () => {
  ...
  test('returns only id, username and email in content array for each user', async () => {
    await addUsers(11);
    const response = await getUsers();
...
describe('Get User', () => {
  ...
  test('returns id, username and email in response body when an active user exists', async () => {
    const user = await User.create({
      username: 'user1',
      email: 'user1@mail.com',
      inactive: false,
